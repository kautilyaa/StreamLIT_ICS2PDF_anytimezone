...
def generate_calendar_pdf(events, overrides, year, month, timezone, color_mapping, is_bw, add_background, background_color):
    ...
    for week_idx, week in enumerate(cal_matrix):
        ...
        for day_idx, day in enumerate(week):
            ...
            if add_background and not is_bw:
                pdf.setFillColor(background_color)
                pdf.rect(x, y - cell_height, cell_w, cell_height, stroke=0, fill=1)
            ...
            if day != 0:
                ...
                if day in events_by_day:
                    ...
                    for event in events_list:
                        ...
                        event_identifier = event.split(" @ ")[1].strip() if " @ " in event else event.split(" - ")[1].strip()
                        if is_bw:
                            pdf.setFillColor(colors.black)
                        else:
                            pdf.setFillColor(color_mapping.get(event_identifier, colors.black))
...
def load_and_process_calendar_data(ics_file, timezone):
    ...
    for component in calendar_data.walk():
        if component.name == "VEVENT":
            ...
            if component.get('SUMMARY'):
                meeting_names.add(component.get('SUMMARY'))
    ...
    return events, overrides, venues, meeting_names
...
def main():
    ...
    if ics_file is not None:
        ...
        color_by = st.selectbox("Color events by", ["Venue", "Meeting Name"])
        ...
        events, overrides, venues, meeting_names = load_and_process_calendar_data(ics_file, timezone)

        if "color_mapping" not in st.session_state:
            color_mapping = {}
            if color_by == "Venue":
                color_mapping = {venue: random_color() for venue in venues}
            else:
                color_mapping = {name: random_color() for name in meeting_names}
            st.session_state["color_mapping"] = color_mapping
        else:
            color_mapping = st.session_state["color_mapping"]

        if color_mode == "Color":
            st.write(f"Customize Colors for {color_by}:")
            for identifier in (venues if color_by == "Venue" else meeting_names):
                current_color = color_mapping[identifier]
                color_hex = '#%02X%02X%02X' % (int(current_color.red * 255), int(current_color.green * 255), int(current_color.blue * 255))
                chosen_color = st.color_picker(f"Pick a color for {identifier}:", value=color_hex)
                color_mapping[identifier] = colors.HexColor(chosen_color)

            add_background = st.checkbox("Add background color to calendar")
            background_color = "#FFFFFF"
            if add_background:
                background_color = st.color_picker("Pick a background color", value="#F0F0F0")
        ...
        while (current_year < end_year) or (current_year == end_year and current_month <= end_month):
            pdf_buffer = generate_calendar_pdf(events, overrides, current_year, current_month, timezone, color_mapping, color_mode == "Black & White", add_background, background_color)
