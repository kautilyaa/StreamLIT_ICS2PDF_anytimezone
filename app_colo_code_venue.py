...
# Define color mapping for venues
venue_colors = {
    "MD Room": colors.blue,
    "Chapel": colors.green,
    
    "1106": colors.purple,
    "Default": colors.black  # Fallback for any venue not listed
}

def generate_calendar_pdf(events, overrides, year, month, timezone):
            ...
            if day != 0:
                ...
                if day in events_by_day:
                    ...
                    for event in events_list:
                        ...
                        # Extract venue from the event details
                        event_location = event.split("@")[-1].strip()

                        # Set color based on venue
                        color = venue_colors.get(event_location, venue_colors["Default"])
...
def main():
    ...
    if ics_file is not None:
        ...
        events, overrides = load_and_process_calendar_data(ics_file, timezone)
        ...
        while (current_year < end_year) or (current_year == end_year and current_month <= end_month):
            pdf_buffer = generate_calendar_pdf(events, overrides, current_year, current_month, timezone)
